...
pb1 = zeros(1,length(r));
pb2 = zeros(1,length(r));
...
for u = 1:length(r)
    ...
    % BASIC FAIR PA %
    aaf_basic = min(1, epsilon*(no + pt.*g1)./(pt.*g1*(1+epsilon)));
    aan_basic = 1 - aaf_basic;
    ...
    gamma_f_basic = pt.*af.*g1./(pt.*g1.*aan_basic + no);
    gamma_nf_basic = pt.*af.*g2./(pt.*g2.*aan_basic + no);
    gamma_n_basic = pt.*g2.*aan_basic/no;
    ...
    Cb_f = log2(1 + gamma_f_basic);
    Cb_nf = log2(1 + gamma_nf_basic);
    Cb_n = log2(1 + gamma_n_basic);
...
plot(r,poutb1,'--*g','linewidth',2);
plot(r,poutb2,'--xm','linewidth',2);
plot(r,pouta1,'r','linewidth',2);
plot(r,pouta2,'b','linewidth',2);
xlabel('Target rate of far user (R*) bps/Hz');
ylabel('Outage probability');
xlim([r(1) r(end)]);
legend('Far user (fixed PA)','Near user (fixed PA)','Far user (basic fair PA)','Near user (basic fair PA)','Far user (improved fair PA)','Near user (improved fair PA)');
